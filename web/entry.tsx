...
const clientRender = async (): Promise<void> => {
  // 客户端渲染||hydrate
  // fixed: https://segmentfault.com/a/1190000012962583
  const useHydrate= process.env.NODE_ENV === 'development' ? !module.hot : window.__USE_SSR__
...
const serverRender = async (ctx: Context): Promise<JSX.Element> => {
  ...
  return <StaticRouter location={ctx.req.url} context={serverData}>
    <Layout layoutData={ctx}>
      <ActiveComponent {...serverData} ctx={ctx} />
    </Layout>
  </StaticRouter>
}
